<?php
    ...
    if (trim($t)==='') {
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            $isunread = $row["status"] == 2 && $id != $row["last_sender_id"];
            $txt_unread = $isunread ? ' unread-txt' : '';
            echo '<div class="lbl search-result" draggable="true">
        <div id="user'.$row["id"].'" class="username-search"><span class="chatname'.$txt_unread.'">'.$row["alias"].'</span> <span class="me" style="color: '.($isunread ? '#0084ff' : '#0006').';" title="'.$row['date'].'">'.$row['time'].'</span></div>
